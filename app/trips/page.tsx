          ...
          <p>
            {trips.length === 0
              ...
              : `You have ${trips.length} ${
                ...
                } saved. ${upcomingTrips.length > 0 ? `${upcomingTrips.length} are upcoming!` : "They're all in the past! Add a new one now."}`}
          </p>
        </CardContent>
      </Card>
      <div>
        <div className="flex items-center justify-between mt-10 mb-5">
          <h2 className="text-xl font-semibold mb-4">Your Next Trips</h2>
          <Link href="/trips/upcoming">
            <Button className="hover:cursor-pointer">See All</Button>
          </Link>
        </div>
        ...
        {upcomingTrips.length === 0 ? (
          ...
          <>
            ...
            <div className="flex items-center justify-between mt-10 mb-5">
              <h2 className="text-xl font-semibold mb-4">Your Past Trips</h2>
              <Link href="/trips/past">
                <Button className="hover:cursor-pointer">See All</Button>
              </Link>
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {pastTrips.slice(0, 6).map((trip, key) => (
                <Link key={key} href={`/trips/${trip.id}`}>
                  <Card className="h-full hover:shadow-md transition-shadow">
                    <CardHeader>
                      <CardTitle className="line-clamp-1">
